    ...
    return (
      <div className="hidden md:flex flex-1 flex-col items-center justify-center bg-gray-50">
        <div className="flex flex-col items-center gap-3 text-center">
          <div className="w-16 h-16 rounded-2xl bg-white border border-gray-200 shadow-sm flex items-center justify-center text-gray-300">
            <ChatIcon />
          </div>
          <div>
      ...
      <div className="flex items-center justify-between px-5 py-3 bg-[#f1f5f9] border-b border-gray-200 flex-shrink-0">
        <div className="flex items-center gap-3">
          <button onClick={() => { setSelectedRoom(null); setSelectedUser(null); }}
            className="md:hidden text-gray-400 hover:text-gray-600 p-1 transition-colors">
            <BackIcon />
          </button>
          <div className="relative flex-shrink-0">
            {!isRoom && currentTarget?.avatar
              ...
              : <div className={`w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-bold ${isRoom ? "bg-blue-100 text-blue-600" : "bg-blue-600"}`}>
                  {isRoom ? <HashIcon /> : currentTarget?.name?.[0]?.toUpperCase()}
                </div>
            }
            {!isRoom && (
              <div className={`absolute bottom-0 right-0 w-2.5 h-2.5 rounded-full border-2 border-[#f1f5f9] ${onlineUserIds?.has(selectedUser?.id) ? "bg-emerald-500" : "bg-gray-300"}`} />
            )}
          </div>
          <div>
            ...
            <p className="text-[10px] mt-0.5">
              {isRoom
                ...
                : <span className={onlineUserIds?.has(selectedUser?.id) ? "text-emerald-500 font-medium" : "text-gray-400"}>
        ...
        <div className="flex items-center gap-1">
          <button className="p-2 rounded-md text-gray-400 hover:text-gray-600 hover:bg-gray-200/60 transition-colors" title="Voice call"><PhoneIcon /></button>
          <button className="p-2 rounded-md text-gray-400 hover:text-gray-600 hover:bg-gray-200/60 transition-colors" title="Video call"><VideoIcon /></button>
        </div>
      </div>
        ...
        {currentMessages.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full gap-2">
            ...
            <p className="text-xs font-semibold text-gray-700">
              {isRoom ? `Welcome to #${selectedRoom.name}` : `Start a conversation with ${selectedUser?.name}`}
            </p>
          ...
          currentMessages.map((msg, i) => {
            ...
            return (
              <div key={msg.id}
                className={`flex items-end gap-2.5 ${isSelf ? "flex-row-reverse" : "flex-row"} ${showAvatar ? "mt-4" : "mt-0.5"}`}
                onMouseEnter={() => setHoveredMsg(msg.id)}
                onMouseLeave={() => setHoveredMsg(null)}>

                {showAvatar ? (
                  <div className={`w-7 h-7 rounded-full flex-shrink-0 flex items-center justify-center text-white text-[10px] font-bold mb-0.5 ${isSelf ? "bg-blue-600" : "bg-gray-300 text-gray-600"}`}>
                    {(msg.sender_name || "U")[0].toUpperCase()}
                  </div>
                ...
                <div className={`flex flex-col max-w-[68%] sm:max-w-[60%] ${isSelf ? "items-end" : "items-start"}`}>
                  {showAvatar && (
                    <div className="flex items-baseline gap-1.5 mb-1">
                      <span className={`text-[11px] font-semibold ${isSelf ? "text-blue-600" : "text-gray-600"}`}>
                        {isSelf ? "You" : msg.sender_name}
                      </span>
        ...
        <div className="px-5 py-2 border-t border-gray-200 bg-white">
          <div className="flex items-center gap-3 bg-gray-50 border border-gray-200 rounded-lg p-2.5 max-w-xs">
            {filePreview
              ...
              : <div className="w-10 h-10 rounded-md bg-blue-50 border border-blue-100 flex items-center justify-center text-blue-400 flex-shrink-0"><FileIcon /></div>
            }
            <div className="flex-1 min-w-0">
      ...
      <div className="px-5 py-3 border-t border-gray-200 bg-[#f1f5f9] flex-shrink-0">
        <div className="flex items-center gap-2 bg-white border border-gray-200 rounded-xl px-3 py-2.5 focus-within:border-blue-400 focus-within:ring-2 focus-within:ring-blue-50 transition-all max-w-5xl mx-auto">
          ...
          <button type="button" onClick={() => fileInputRef.current.click()}
            className="text-gray-400 hover:text-blue-500 transition-colors flex-shrink-0 p-0.5" title="Attach file">
            <AttachIcon />
          </button>
        ...
        <p className="text-[10px] text-gray-400 mt-1.5 text-center">
          Press <kbd className="bg-white border border-gray-200 rounded px-1 py-0.5 text-[9px] text-gray-500">Enter</kbd> to send
        </p>
      </div>
    </div>
  );
